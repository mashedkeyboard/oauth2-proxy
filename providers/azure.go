...
func (p *AzureProvider) verifyTokenAndExtractEmail(ctx context.Context, token string, accessToken string) (string, error) {
	email := ""
	...
	if token != "" && p.Verifier != nil {
		...
		if err == nil {
			s, err := p.buildSessionFromClaims(token, accessToken)
			if err == nil {
				email = s.Email
			} else {
